...
function getEtiquetaWordItemsSync() {
  ...
  return [...stockItems, ...historicoItems]
    .map(item => {
...
function renderEtiquetasWord(lista = getEtiquetaWordItemsSync()) {
  ...
  host.innerHTML = lista.map((item, idx) => `
    <div class="etiqueta-card">
      ...
      <div class="card-actions">
        <button class="small-btn btn-edit" data-regerar-etq="${idx}">Regerar Etiqueta</button>
        <button class="small-btn btn-delete" data-apagar-etq="${idx}">Apagar</button>
      </div>
    </div>`).join("");
...
function filtrarEtiquetasWord() {
  ...
  const filtrada = getEtiquetaWordItemsSync().filter(item => normalizarTexto([
    item.localCurto,
    item.serie,
    item.armazem,
    item.cor,
...
async function regerarEtiquetaWordSync(item) {
  ...
  if (typeof gerarWordEtiquetaFromForm === "function") {
    await gerarWordEtiquetaFromForm(false);
  }
}
